...
export default class DatabaseUtil {
    ...
    public static addExecutedCommand(amount: number): void {
        ...
        SqLite3.prepare("INSERT OR REPLACE INTO statistics (version, commandsExecuted, songsPlayed, playlistsPlayed) VALUES (?, ?, ?, ?)").run("v3", currentCommands + amount, currentSongs, currentPlaylists);
    }
    ...
    public static addPlayedSong(amount: number): void {
        ...
        const currentSongs = this.getTotalSongsPlayed();
        const currentPlaylists = this.getTotalPlaylistsPlayed();
        SqLite3.prepare("INSERT OR REPLACE INTO statistics (version, commandsExecuted, songsPlayed, playlistsPlayed) VALUES (?, ?, ?, ?)").run("v3", currentCommands, currentSongs + amount, currentPlaylists);
    }
    ...
    public static addPlaylistPlayed(amount: number): void {
        ...
        const currentSongs = this.getTotalSongsPlayed();
        const currentPlaylists = this.getTotalPlaylistsPlayed();
        SqLite3.prepare("INSERT OR REPLACE INTO statistics (version, commandsExecuted, songsPlayed, playlistsPlayed) VALUES (?, ?, ?, ?)").run("v3", currentCommands, currentSongs, currentPlaylists + amount);
    }

    public static getTotalCommandsExecuted(): number {
        const total = SqLite3.prepare("SELECT commandsExecuted FROM statistics WHERE version=?").get("v3");
        return total.commandsExecuted;
    }

    public static getTotalSongsPlayed(): number {
        const total = SqLite3.prepare("SELECT songsPlayed FROM statistics WHERE version=?").get("v3");
        return total.songsPlayed;
    }

    public static getTotalPlaylistsPlayed(): number {
        const total = SqLite3.prepare("SELECT playlistsPlayed FROM statistics WHERE version=?").get("v3");
